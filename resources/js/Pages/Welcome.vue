<template>
    <div class="max-w-sm sm:max-w-md md:max-w-lg w-full">
        ...
        <div v-if="posts.length > 0">
            <post-component-vue v-for="(publicaciones, i) in posts" :key="i" :post="publicaciones"
                @dataPost="setPost" />
        </div>
        ...
        <ModalVue :show="showModalCreate" @close="changeStateShowCreatePost">
            <div class="p-5" id="modal_publicar_post">
                <div>
...
export default {
    ...
    methods: {
        async getPost() {
            //obteniendo todos las publicaciones
    ...
    created() {
        //esto es como lo de jquery que se ejecuta al cargar
        this.getPost();
    },
};
</script>
