  ...
  <div class="bg-gray-100 mt-4">
    <div class="bg-white border rounded-sm max-w-xl2">
      <div class="flex items-center px-4 py-3">
        ...
        <div class="ml-3">
          <span class="text-sm font-semibold antialiased block leading-tight">{{ post.user.nick_name }}</span>
          <span class="text-gray-600 text-xs block">{{ getDifferentTime(post.created_at) }}</span>
        </div>
      </div>
      ...
      <div class="font-semibold text-sm mx-4 mt-2">{{ post.countLikes }} likes</div>
      <div class="font-black text-sm mx-4 mt-2 ">{{ post.description }}</div>
    
      
      <CommentsVue :comment="post.description" :nick_name="post.user.nick_name" :urlImage="post.user.profile_photo_url"/>
